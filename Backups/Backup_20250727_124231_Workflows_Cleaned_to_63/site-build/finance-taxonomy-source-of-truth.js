...
exports.FINANCE_TAXONOMY = void 0;
exports.validateTaxonomy = validateTaxonomy;
exports.FINANCE_TAXONOMY = {
    functionalAreas: 7,
    totalActivities: 63,
    totalPlatforms: 17,
    structure: {
...
function validateTaxonomy() {
    var areas = Object.values(exports.FINANCE_TAXONOMY.structure);
    // Check functional area count
    if (areas.length !== exports.FINANCE_TAXONOMY.functionalAreas) {
        console.error("Functional area mismatch: Expected ".concat(exports.FINANCE_TAXONOMY.functionalAreas, ", got ").concat(areas.length));
        return false;
    }
    // Check total activities
    var totalActivities = areas.reduce(function (sum, area) { return sum + area.activityCount; }, 0);
    if (totalActivities !== exports.FINANCE_TAXONOMY.totalActivities) {
        console.error("Activity count mismatch: Expected ".concat(exports.FINANCE_TAXONOMY.totalActivities, ", got ").concat(totalActivities));
        return false;
    }
    // Check total platforms
    var totalPlatforms = areas.reduce(function (sum, area) { return sum + area.platformCount; }, 0);
    if (totalPlatforms !== exports.FINANCE_TAXONOMY.totalPlatforms) {
        console.error("Platform count mismatch: Expected ".concat(exports.FINANCE_TAXONOMY.totalPlatforms, ", got ").concat(totalPlatforms));
        return false;
    }
    // Verify each area's counts match
    for (var _i = 0, _a = Object.entries(exports.FINANCE_TAXONOMY.structure); _i < _a.length; _i++) {
        var _b = _a[_i], areaName = _b[0], area = _b[1];
        if (area.activities.length !== area.activityCount) {
            console.error("".concat(areaName, ": Activity count mismatch"));
            return false;
        }
        if (area.platforms.length !== area.platformCount) {
            console.error("".concat(areaName, ": Platform count mismatch"));
            return false;
        }
    }
    return true;
