...
# Read the file
with open('investor-relations-workflows.ts', 'r') as f:
    content = f.read()
...
def replace_agent_ids(match):
    ...
    # Generate new IDs with IR- prefix
    new_agents = []
    for agent in agents:
        new_id = f"IR-{agent}"
        # Add unique suffix if needed
        if new_id in new_agents:
            new_id = get_next_id(f"IR-{agent}")
...
# Write back
with open('investor-relations-workflows.ts', 'w') as f:
    f.write(content)

print("Updated all agent IDs in investor relations workflows")
